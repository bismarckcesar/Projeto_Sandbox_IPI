<?php
	$stmt = $con->prepare("SELECT EATING_PLANS.DATE_START,  EATING_PLANS.DATE_FINISH , EATING_PLANS.OBJECTIVE, EATING_PLANS.ID, EATING_PLANS.PATIENT_ID, NUTRITIONISTS.NAME FROM EATING_PLANS INNER JOIN NUTRITIONISTS ON (EATING_PLANS.NUTRITIONIST_ID = ? OR EATING_PLANS.PATIENT_ID = ?) ORDER BY DATE_START");
